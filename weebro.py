		...
		for element in elements_in_view_port:
			...
			if inner_text != "": 
				elements_of_interest.append(
					f"""<{converted_node_name} {id_counter}{meta}>{inner_text}</{converted_node_name}>"""
				)
			else:
				elements_of_interest.append(
					f"""<{converted_node_name} {id_counter}{meta}/>"""
				)
			id_counter += 1
...
if (__name__ == "__main__"):
	...
	def get_gpt_command(objective, url, previous_command, browser_content):
		...
		# response = client.completions.create(model="text-davinci-002", prompt=prompt, temperature=0.5, best_of=10, n=3, max_tokens=50)  # Updated method call
		...
		while True:
			...
			if run_status.status in ["completed"]:
				break
			elif run_status.status in ["cancelled", "failed", "expired", "requires_action"]:
				error_message = "Run encountered a problem: " + run_status.status
				if run_status.last_error is not None:
					error_message += f"\nError Code: {run_status.last_error.code}"
	...
	def run_cmd(cmd):
		...
		if cmd.startswith("SCROLL UP"):
			...
			spacesplit = cmd.split(" ")
			_crawler.go_to_page(spacesplit[1])
	...
	try:
		while True:
			...
			# gpt_cmd = gpt_cmd[0].text.value.strip()
